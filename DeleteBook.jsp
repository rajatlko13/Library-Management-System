...
<div class="jumbotron text-center mb-0 mt-2 bg-dark text-warning" >
  ...
  <%
	  try
	  {
		...
		String query="select * from originalbooklist";
...
<span class="font-weight-bold text-white">Book Id :<span> 
<select name="bookid" class="py-1 px-2 mr-3">
<% while(rs.next()) { %>
    <option><%=rs.getString("id") %></option>
<% } %>
 </select>
<span class="font-weight-bold text-white">Copies :<span>  <input type="number" name="copies" style="width:110px; text-align:center" placeholder="Enter copies" class="mr-2 py-1">
<button class="btn btn-danger">Delete Books</button>
</form>
</div>

<table class="table container text-center bg-dark text-light">
